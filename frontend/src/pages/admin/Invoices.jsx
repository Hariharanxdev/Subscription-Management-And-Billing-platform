...
export default function AdminInvoices() {
  ...
  const columns = [
    ...
    { key: "payment_id", header: "Payment", render: (r) => <span className="tabular">#{r.payment_id}</span> },
  ...
  if (error) return <ErrorState message="Couldn't load invoices." onRetry={load} />;

  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-ink">Invoices</h2>
        <p className="mt-1 text-sm text-ink-faint">Every invoice generated across all customers.</p>
      </div>
